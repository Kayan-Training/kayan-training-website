...
const agendaItemSchema = z.object({
  ...
  speakerNamesEn: z.array(z.string()),
  speakerNamesAr: z.array(z.string()),
...
function flattenErrorPaths(
  ...
  for (const [key, value] of Object.entries(errors)) {
    ...
    const maybeError = value as { message?: unknown; type?: unknown };
    if (
...
function AgendaSpeakersCombobox({
  options,
  value,
  onChange,
  invalid = false,
  ...
  return (
    <div className="space-y-1 px-2 py-1.5">
      <Popover onOpenChange={setOpen} open={open}>
        <PopoverTrigger
          className={cn(
            "h-8 w-full rounded-md border border-zinc-200 bg-white px-2 text-left text-[11px] text-zinc-700 hover:border-zinc-300",
        ...
        >
          <span className="line-clamp-1">
            {value.length
              ? `${value.length} speaker${value.length > 1 ? "s" : ""} selected`
              : "Add speakers"}
          </span>
        </PopoverTrigger>
        <PopoverContent align="start" className="w-[280px] p-0">
          <Command>
            <CommandInput
              placeholder="Search or type speaker name..."
              value={query}
            ...
            <CommandList>
              ...
              {normalized ? (
                <CommandGroup heading="Create">
                  <CommandItem onSelect={() => addSpeaker(normalized)}>
                    Add "{normalized}"
                  </CommandItem>
                </CommandGroup>
              ) : null}
              <CommandGroup heading="Trainers">
                {available.map((item) => (
                  <CommandItem key={item} onSelect={() => addSpeaker(item)}>
                    {item}
                  </CommandItem>
                ))}
              </CommandGroup>
      ...
      {value.length ? (
        <div
          className={cn(
            "max-h-14 overflow-y-auto rounded-md border border-zinc-200 bg-zinc-50 p-1 no-scrollbar",
...
export function EventForm({
  ...
  const [galleryLibraryTotalPages, setGalleryLibraryTotalPages] = useState(1);
  ...
  const normalizedAgendaDefaults = (defaultValues?.agenda ?? []).map((item) => {
    ...
    return {
      ...
      speakerNamesEn: item.speakerNamesEn ?? legacySpeakers ?? [],
      speakerNamesAr: item.speakerNamesAr ?? [],
  ...
  const watchedAgenda = useWatch({ control: form.control, name: "agenda" });
  ...
  const agendaInvalidTitleCount = useMemo(
    () =>
      (watchedAgenda ?? []).filter(
        (item) =>
          !(item?.titleEn ?? "").trim().length &&
          !(item?.titleAr ?? "").trim().length,
      ).length,
    [watchedAgenda],
  );
  const formErrorPaths = useMemo(
    () => flattenErrorPaths(form.formState.errors),
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const parsed = JSON.parse(raw) as EventFormValues;
      form.reset(parsed);
  ...
  }, [draftStorageKey, form, form.formState.isDirty, watchedFormValues]);
  ...
  async function openCoverLibrary(
    target: "cover" | "heroProgram" | "heroCollaborators" = "cover",
  ) {
    ...
    <>
      <form
        ...
        onSubmit={form.handleSubmit(save, (errors) => {
          ...
          const first = paths[0] ?? "";
          if (first.startsWith("agenda.")) {
            setActiveSection("agenda");
            const idx = Number(first.split(".")[1]);
            if (!Number.isNaN(idx)) {
              const row = form.getValues(`agenda.${idx}`);
              if (row?.day) setActiveDay(row.day);
            }
          } else if (
            ...
            first.startsWith("endDate") ||
            first.startsWith("registrationDeadline") ||
            ...
            first.startsWith("price") ||
            first.startsWith("isFree") ||
            first.startsWith("payment") ||
            first.startsWith("bank") ||
            first.startsWith("registrationType") ||
          ...
          const shortList =
            paths.length > 0
              ? `Missing/invalid: ${paths.slice(0, 4).join(", ")}${paths.length > 4 ? "..." : ""}`
              : "Please fix invalid fields before saving.";
          toast.error(shortList);
        })}
      >
        {/* ════════════════════════════════════════════════════════════════
            LEFT NAV RAIL  w-52
                          ...
                          <div className="mt-6 space-y-4">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-muted-foreground text-xs">
                                  Add tiers (e.g. Early Bird, Standard) to
                                  replace the single price above with pricing
                                  cards. Leave empty to keep using the single
                                  price field.
                                </p>
                              </div>
                              <Button
      ...
      <MediaLibraryDialog
        ...
        onConfirm={(selected) => {
          ...
          const merged = Array.from(
            new Set([...heroCollaboratorLogos, ...urls]),
          );
          form.setValue("heroCollaboratorLogos", merged.join("\n"), {
            shouldDirty: true,
          });
