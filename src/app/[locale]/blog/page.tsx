...
export default async function PostsPage({
  ...
  const { locale } = await params;
  const activeLocale = isSupportedLocale(locale) ? locale : "ar";
  const listingConfig = (await getListingConfig(activeLocale, "blog")) ?? (await getListingConfig(activeLocale, "posts"));
  ...
  return (
    <main className="pt-16">
      ...
      <section className="mx-auto w-full max-w-[1440px] px-6 py-12 md:px-10">
        {featured ? (
          <Link className="group relative mb-10 block min-h-[460px] overflow-hidden ghost-border" href={`/${activeLocale}/blog/${featured.slug}`}>
            <Image
              alt=""
              className="absolute inset-0 h-full w-full scale-105 object-cover grayscale transition-all duration-700 group-hover:scale-100 group-hover:grayscale-0"
              fill
